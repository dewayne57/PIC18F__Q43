...
void SYSTEM_Initialize(void)
{
   ...
   ADCON0 = 0x00;       // Reset ADCCON0 to default
   ADCON0bits.CONT = 1; // Continuous conversion mode
   //ADCON0bits.CONT = 0; // Not continuous conversion 
   ADCON0bits.FM = 1;   // Right justified result format
   ADCON0bits.CS = 0;   // Clock source = FOSC/64 (default)

   ADCON1 = 0; // Reset ADCCON1 to default (no precharge or guard rings)

   ADCON2 = 0;                                  // Reset ADCCON2 to default
   //ADCON2bits.CRS = ADCC_AVERAGE_DIVISOR_SHIFT; // Set the shift amount for averaging

   ADCON3 = 0x00;             // Reset ADCON3 to default
   ...
   //ADRPT = ADCC_OVERSAMPLE_COUNT - 1; // Set oversample count for averaging
   ADRPT  = 0;
   ADCNT = 0;                          // Clear the ADC conversion count
   //ADCON2bits.MD = 0b010;             // Set ADC mode to continuous conversion with oversampling
                                       // and averaging
   ADCON2bits.MD = 0b000;                // Legacy mode
   //ADPRE = ADCC_PRECHARGE_INTERVAL;    // Set precharge interval for ADC
   ADPRE = 0;
   ADACQ = ADCC_ACQUISITION_INTERVAL;  // Set acquisition interval for ADC
   ...
   ADCON0bits.ON = 1; // Enable the ADC module
   ADCON0bits.GO = 1; // Start the first ADC conversion
