...
void UART1_Initialize(void)
{
    ...
    /* Async mode, 8-bit, no parity, 1 stop-bit, no HW flow control (2-wire UART). */
    U1CON2 = 0x00;
    U1CON2bits.RUNOVF = 1; /* Enable receiver buffer overflow detection */
    U1CON2bits.FLO = 0;    /* No flow control   */
    U1BRG = (uint16_t)UART_1_BRG_VALUE;

    U1ERRIR = 0x00;          /* Clear all error flags */
    U1ERRIEbits.U1TXMTIE = 1; /* Enable TX empty interrupt */

    PIE4bits.U1TXIE = 0;    /* Disable transmit interrupt until we have data to send. */
    PIE4bits.U1RXIE = 0;    /* Disable receive interrupt to receive incoming data for now. */
    ...
    U1CON0bits.TXEN = 1; 
    U1CON1 = 0x00;
    U1CON1bits.ON = 1; /* UART module on */ 
    __delay_ms(10);
    char dummy = U1RXB; /* Clear any transient data */ 
    PIE4bits.U1RXIE = 1; /* Enable the receive data interrupts */ 
