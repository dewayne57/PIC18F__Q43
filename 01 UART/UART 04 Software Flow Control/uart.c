...
/* NOTE: RTS/CTS levels are defined here for reference but are not used in this project.
   XON/XOFF software flow control does not require dedicated hardware handshake lines. */
...
void UART1_Initialize(void)
{
    ...
    /* Async mode from config, software XON/XOFF flow control. */
    U1CON2 = 0x00;
    U1CON2bits.RUNOVF = 1; /* RUNOVF=1: receiver keeps running and flags an error on overflow
                              rather than halting, to avoid silently dropping incoming bytes */
    /* FLO selects UART1 flow control mode:
       0 = none (simple 2-wire, no flow control)
       1 = software XON/XOFF - UART hardware automatically sends XON (0x11) or XOFF (0x13)
           characters when the receive buffer fills up or drains.  The remote device must
           pause sending when it receives XOFF and resume when it receives XON.
       2 = hardware RTS/CTS (UART 03 uses this) */
    U1CON2bits.FLO = 1;    /* Software XON (0x11) / XOFF (0x13) flow control mode */
    ...
    /* TX interrupt starts disabled - enabled by putch() only after data is queued.
       Enabling it now with an empty buffer would trigger an immediate spurious ISR. */
    ...
    U1CON0 = 0x00;
    /* U1CON0.MODE selects UART framing format:
       0x0 = 8-bit, no parity (default / most common)
       0x2 = 9-bit data
       0x8 = 8-bit with odd parity
       0x9 = 8-bit with even parity */
    U1CON0bits.MODE = 0x0;  // Default: 8-bit, no parity

    if (UART_1_DATA_BITS == 9)
    {
        U1CON0bits.MODE = 0x2;  // 9-bit data mode
    }
    ...
    /* U1CON2.STP selects stop bit count:
       ...
       UART_STOP_BITS_2   = 2 -> 2 stop bits (noisy lines or legacy protocols) */
    ...
    U1FIFObits.XON = 1;     // Start software flow control in the "ready to receive" state
    __delay_ms(10);         // Brief settling delay before first byte
...
static uint8_t UART1_TxBufferFreeCount(void)
{
    ...
    /* When head >= tail, data occupies bytes from tail to head (no wrap).
    ...
    /* One slot is kept empty to distinguish "empty" (head==tail) from "full". */
...
{
    /* Advance head by 1 using bitmask wrap (works because SIZE is a power of 2). */
    uint8_t next_head = (uint8_t)((tx_head + 1U) & UART1_TX_BUFFER_LIMIT);
    if (next_head == tx_tail)   // Buffer would be full - reject the byte
    {
        return false;
    }
...
{
    /* NOTE: In XON/XOFF mode the UART hardware intercepts XON (0x11) and XOFF (0x13)
       characters and uses them to pause/resume TX automatically; they are not passed to
       the receive buffer.  Any other received byte comes through here as normal.
       Discard null bytes (0x00) - these are power-up glitch artifacts, not real data. */
    if (data == 0x00) { 
        return true;
    }
    
    uint8_t next_head = (uint8_t)((rx_head + 1U) & UART1_RX_BUFFER_LIMIT); // Circular advance
    if (next_head == rx_tail)   // Buffer full - drop incoming byte rather than overwriting unread data
    {
        return false;
    }
...
void putch(char data)
{
    if (uart1_initialized)
    {
        ...
        (void)UART1_TxBufferPush(data);   // Queue the character in the ring buffer

        /* Prime the pump: if the hardware FIFO is idle and TX interrupt is off, manually
           send the first byte to kick off the ISR-driven transmission chain. */
        if (U1FIFObits.TXBE == 1 && PIE4bits.U1TXIE == 0U)
        {
            UART1_SendNext();  // Start the TX interrupt chain
        }
    }
}
