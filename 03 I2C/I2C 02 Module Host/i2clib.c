...
{
    i2c_address7_t addr;
    // Caller provides the 8-bit write-form address; update bit0 for R/W.
    addr.address_l = (uint8_t)((device_address & 0xFEU) | (read ? 1U : 0U));
    return addr;
}
...
/// @brief Arms the read phase of a write-read transaction exactly once.
/// @return true if the transition was armed by this call; false otherwise.
static bool i2c_tryArmWriteReadReadPhase(void)
{
    if ((active_handle == NULL) ||
        (active_handle->current_operation != I2C_OP_WRITE_READ) ||
        (!active_handle->repeated_start_pending))
    {
        return false;
    }
    ...
    case I2C_MODE_HOST_7BIT:
        active_handle->device_address.address7.address_l |= 0x01U;   // Set R/W bit (bit0) for read
        I2C1ADB1 = active_handle->device_address.address7.address_l; // Load read address into ADB1
        break;
    ...
    I2C1CNT = (uint8_t)active_handle->rx_buffer_size; // Load read byte count
    I2C1CON0bits.RSEN = 1;                             // Keep repeated-start request asserted
    I2C1CON0bits.CSTR = 0;                             // Release any clock-stretch hold before restart
    active_handle->rx_pos = 0;
    active_handle->repeated_start_pending = false;
    active_handle->current_operation = I2C_OP_READ;
    return true;
}
...
i2c_status_t i2c_init(i2c_handle_t *handle, uint8_t channel, i2c_mode_t mode, uint16_t speed)
{
    ...
    I2C1ERR = 0x00;         // Clear all pending error conditions and source enables
    I2C1ERRbits.NACKIE = 1; // Enable NACK error source
    I2C1ERRbits.BCLIE = 1;  // Enable bus collision error source
    I2C1ERRbits.BTOIE = 1;  // Enable bus-timeout error source
    I2C1PIE = 0x00;         // Clear module event interrupt enables before configuration
    I2C1PIEbits.SCIE = 1;   // Raise general I2C IRQ on START condition events
    I2C1PIEbits.RSCIE = 1;  // Raise general I2C IRQ on repeated START condition events
    I2C1PIEbits.PCIE = 1;   // Raise general I2C IRQ on STOP condition events
    I2C1PIEbits.ACKTIE = 1; // Raise general I2C IRQ on ACK timing events
    I2C1PIEbits.CNTIE = 1;  // Raise general I2C IRQ when the transfer byte count reaches zero

    IPR7bits.I2C1IP = 1;   // I2C General/event interrupt is high priority
    IPR7bits.I2C1EIP = 1;  // I2C Error interrupt is high priority
    IPR7bits.I2C1RXIP = 1; // I2C Receive interrupt is high priority
    IPR7bits.I2C1TXIP = 1; // I2C Transmit interrupt is high priority

    PIE7bits.I2C1IE = 1;   // General/event IRQ handles CNTIF transfer completion
    PIE7bits.I2C1EIE = 1;  // Enable vectored i2c error interrupts
    PIE7bits.I2C1RXIE = 1; // Enable vectored i2c RX interrupts
    ...
    // (e.g., set baud rate, enable host mode, etc.)
    case I2C_MODE_HOST_7BIT:
    case I2C_MODE_HOST_10BIT:

        // compute the necessary clock source to use for the selected speed.
        ...
        // Keep MODE from the assignment above; clearing I2C1CON0 here would force MODE=0.
        ...
        I2C1CON2bits.SDAHT = 1; // Set SDA hold time to 300ns
        break;
                             ...
                             uint8_t length)
{
    ...
    if ((active_handle != NULL) || (handle->current_operation != I2C_OP_NONE))
    {
        return I2C_ERROR_ILLEGAL_STATE; // A transfer is already in progress
    }
    ...
    I2C1CON0bits.RSEN = 0; // A plain write must not inherit a pending repeated-start request.
    handle->tx_buffer = data;
    handle->tx_buffer_size = length;
    handle->tx_buffer_pos = 0;
    handle->repeated_start_pending = false;
    ...
    // CNT is total payload byte count (includes the preloaded TXB data byte).
    I2C1CNT = length;
                            ...
                            uint8_t length)
{
    ...
    I2C1CON0bits.RSEN = 0; // A plain read must not inherit a pending repeated-start request.
    ...
    case I2C_MODE_HOST_7BIT:
    {
        i2c_address7_t addr7 = format_7bit_address((uint8_t)address, true); // Format the 7-bit address with the read bit
        active_handle->device_address.address7 = addr7;                      // Store the formatted address in the handle for ISR access
        I2C1ADB1 = addr7.address_l;                                          // Load the 7-bit address into ADB1, ensuring the R/W bit is cleared for write
        I2C1ADB0 = 0x00;                                                     // Clear ADB0 since we're using ADB1 for the address in 7-bit host mode
        break;
    ...
    case I2C_MODE_HOST_10BIT:
    {
        i2c_address10_t addr10 = format_10bit_address(address, true); // Format the 10-bit address with the read bit
        active_handle->device_address.address10 = addr10;              // Store the formatted address in the handle for ISR access
        I2C1ADB0 = addr10.address_l;                                   // Load the low byte of the 10-bit address into ADB0
        I2C1ADB1 = addr10.address.address_h;                           // Load the high part of the 10-bit address into ADB1
        break;
    ...
    I2C1CNT = length;   // Load the byte count (not including address)
...
/// @brief Performs a combined write-read transaction to the specified I2C
/// client device. This function initiates an I2C write transaction to the
/// given address, transmitting the specified data, followed by an I2C read
/// transaction to receive data from the same address. The function will handle
/// the necessary I2C protocol steps, including generating the start condition,
/// transmitting the address and write data bytes, generating a repeated start
/// condition, transmitting the address with the read bit, receiving the read
/// data bytes, and generating the stop condition. The received data will be
/// stored in the provided buffer, and the status of the operation will be
/// returned to indicate success or any errors that may have occurred during the
/// transaction.
/// @param handle Pointer to the I2C handle structure.
/// @param address The 7-bit I2C address of the target device.
...
/// @param read_data Pointer to the buffer where the received data will be stored in the read phase.
/// @param read_length Number of bytes to be received in the read phase.
/// @return The status of the I2C write-read operation.
i2c_status_t i2c_writeReadClient(i2c_handle_t *handle, uint16_t address, uint8_t write_byte,
                                 uint8_t *read_data, uint8_t read_length)
{
    ...
    I2C1CON0bits.RSEN = 0; // Reset restart request first; writeRead is the only path that re-arms it.
    handle->tx_buffer = NULL;
    handle->tx_buffer_size = 1;
    handle->tx_buffer_pos = 0;
    handle->rx_buffer = read_data;
    handle->rx_buffer_size = read_length;
    handle->rx_buffer_pos = 0;
    handle->repeated_start_pending = true;
    ...
    // CNT is total payload byte count (includes the preloaded TXB data byte).
    I2C1CNT = 1;          // One write byte precedes the repeated start
    I2C1TXB = write_byte; // Prime the register address / command byte
    handle->tx_pos = 1;   // Write phase is fully queued
    I2C1CON0bits.RSEN = 1; // Arm repeated-start request before starting the write phase
    I2C1CON0bits.S = 1;    // Assert the start condition to begin the transfer

    return I2C_SUCCESS; // Return success status for now (actual implementation needed)
}

/// @brief Checks if the current I2C transfer associated with the handle is complete.
/// This function can be used by the user to poll the status of an ongoing I2C transfer
/// to determine if it has completed.
/// @param handle Pointer to the I2C handle structure.
/// @return true if the transfer is complete, false if the transfer is still in progress
/// or if the handle is not initialized.
bool i2c_isComplete(i2c_handle_t *handle)
{
    if ((handle == NULL) || (!handle->initialized))
    {
        return false; // Handle is not initialized, cannot determine completion status
    }
    
    return (handle->current_operation == I2C_OP_NONE); // Transfer is complete if there is no current operation
}

/// @brief Waits for the current I2C transfer associated with the handle to complete. This
/// function will block execution until the transfer is complete, allowing the user to wait
/// for the completion of an I2C operation before proceeding with further actions. The function
/// will check the status of the transfer using the i2c_isComplete function and will return
/// once the transfer is finished.
/// @param handle Pointer to the I2C handle structure.
/// @return The status of the wait operation, indicating success or if the handle was not
/// initialized.
i2c_status_t i2c_waitComplete(i2c_handle_t *handle)
{
    if ((handle == NULL) || (!handle->initialized))
    {
        return I2C_ERROR_NOT_INITIALIZED; // Handle is not initialized, cannot wait for completion
    }

    uint16_t loops = 0;
    while (!i2c_isComplete(handle))
    {
        __delay_us(100);
        loops++;
        if (loops >= 60000U) // ~6 seconds total wait
        {
            handle->status = I2C_ERROR_TIMEOUT;
            i2c_printDiagnostics(handle, "waitComplete timeout");
            return I2C_ERROR_TIMEOUT;
        }
    }

    return I2C_SUCCESS; // Transfer completed successfully
}

/// @brief Print a concise diagnostics dump of the I2C software state and key
/// I2C1 hardware registers.
...
void i2c_printDiagnostics(i2c_handle_t *handle, char* title)
{
    ...
    printf("handle=%s active=%s\r\n",
           (handle != NULL) ? "set" : "null",
           (active_handle != NULL) ? "set" : "null");

    if (handle != NULL)
    {
        printf("init=%u sig=0x%04X mode=%u ch=%u speed_khz=%u\r\n",
               (unsigned)handle->initialized,
               (unsigned)handle->signature,
               (unsigned)handle->mode,
               (unsigned)handle->channel,
               (unsigned)handle->speed_khz);
        printf("status=%u op=%u retry=%u\r\n",
               (unsigned)handle->status,
               (unsigned)handle->current_operation,
               (unsigned)handle->retry_count);
        printf("tx_size=%u tx_pos=%u tx_buf_pos=%u\r\n",
               (unsigned)handle->tx_buffer_size,
               (unsigned)handle->tx_pos,
               (unsigned)handle->tx_buffer_pos);
        printf("rx_size=%u rx_pos=%u rx_buf_pos=%u\r\n",
               (unsigned)handle->rx_buffer_size,
               (unsigned)handle->rx_pos,
               (unsigned)handle->rx_buffer_pos);
    }

    printf("CON0=0x%02X CON1=0x%02X CON2=0x%02X CLK=0x%02X CNT=%u\r\n",
           (unsigned)I2C1CON0,
           (unsigned)I2C1CON1,
           (unsigned)I2C1CON2,
           (unsigned)I2C1CLK,
           (unsigned)I2C1CNT);
    printf("STAT0=0x%02X STAT1=0x%02X ERR=0x%02X PIR=0x%02X PIE=0x%02X\r\n",
           (unsigned)I2C1STAT0,
           (unsigned)I2C1STAT1,
           (unsigned)I2C1ERR,
           (unsigned)I2C1PIR,
           (unsigned)I2C1PIE);
    printf("ADB1=0x%02X ADB0=0x%02X TXB=0x%02X RXB=0x%02X\r\n",
           (unsigned)I2C1ADB1,
           (unsigned)I2C1ADB0,
           (unsigned)I2C1TXB,
           (unsigned)I2C1RXB);
    printf("PIR7=0x%02X PIE7=0x%02X IPR7=0x%02X\r\n",
           (unsigned)PIR7,
           (unsigned)PIE7,
           (unsigned)IPR7);
    printf("[I2C DIAG] --------------------------------\r\n");
...
/// @brief The I2C "General" interrupt service routine. This ISR handles module event
/// conditions exposed through I2C1PIR. We use CNTIF as the authoritative indication that
/// the current transfer byte count has reached zero.
void __attribute__((weak)) __interrupt(irq(IRQ_I2C1), high_priority) i2c1_generalISR(void)
{
    // Debug LED map for general events:
    // LED0 = START detected/generated
    // LED1 = repeated START detected/generated
    // LED2 = STOP detected/generated
    // LED3 = transfer count reached zero
    // LED4 = ACK timing event detected
    // LED5 = write-read transition armed (read phase prepared)
    // LED6 = stale write-phase CNTIF ignored
    // LED7 = true operation completion (WRITE or READ)
    if (I2C1PIRbits.SCIF)
    {
        debug_led0(true);
        I2C1PIRbits.SCIF = 0;
    ...
    {
        debug_led4(true);
        I2C1PIRbits.ACKTIF = 0; // Clear the module ACK timing condition
    }

    if (I2C1PIRbits.CNTIF)
    {
        debug_led3(true);
        I2C1PIRbits.CNTIF = 0; // Clear the module count-complete condition

        // For write-read transfers, count-complete of the write byte is the boundary where
        // the peripheral can be armed for the repeated-start read phase without injecting
        // extra write data.
        if (i2c_tryArmWriteReadReadPhase())
        {
            debug_led5(true);
            PIR7bits.I2C1IF = 0;
            return;
        ...
        if ((active_handle != NULL) &&
            (active_handle->current_operation == I2C_OP_WRITE))
        {
            debug_led7(true);
            active_handle->current_operation = I2C_OP_NONE;
            active_handle = NULL;
        }
        else if ((active_handle != NULL) &&
                 (active_handle->current_operation == I2C_OP_READ))
        {
            // Ignore stale CNTIF from the completed write phase of a write-read transfer.
            // Final completion of reads is confirmed once RX has filled the caller buffer.
    ...
    PIR7bits.I2C1IF = 0; // Clear top-level general interrupt flag
}

/// @brief The I2C "Transmit" interrupt service routine.
/// This interupt is generated whenever the I2C1CNT (count) register is not zero and the
/// transmit buffer is empty, indicating that the next byte can be transmitted. The ISR will need
/// to check the current state of the transfer, load the next byte to transmit into the appropriate
/// register, and update the transfer status accordingly. The ISR may also need to check for
/// conditions such as the byte count being complete, a repeated start condition being needed for
/// a write-read operation, or an error condition occurring during transmission, and take
...
void __attribute__((weak)) __interrupt(irq(IRQ_I2C1TX), high_priority) i2c1_transmitISR(void)
{
    ...
    {
        // Everything to transmit has been sent.
        // For write-read transfers, the CNTIF path arms the read phase.
...
void __attribute__((weak)) __interrupt(irq(IRQ_I2C1RX), high_priority) i2c1_receiveISR(void)
{
    ...
    I2C1CON0bits.RSEN = 0; // Restart disabled
    active_handle->repeated_start_pending = false;
