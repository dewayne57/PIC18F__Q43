/* *****************************************************************************************
 *   File Name: app.c
 *   Description: Hardware I2C1 module host implementation for PIC18F__Q43.
 ...
 *
 *   This is the demonstration application implementation for the I2C 02 Module Host
 *   example. It initializes the system, sets up UART1 for debug output, and configures
 *   the I2C1 hardware module host interface.
 *
 *   This application uses an MCP23017 I2C I/O expander as a remote client device for
...
///       hardware module host interface. It also prints status messages to the console.
void APP_Initialize(void)
{
    printf("I2C 02 Module Host\r\n");
    __delay_ms(500);
    ...
    // Initialize I2C1 hardware module host for 100kHz bus speed
    if (i2c_init(&i2c_host, 1, I2C_MODE_HOST_7BIT, 100) != I2C_SUCCESS)
    {
    ...
    __delay_ms(500);
    i2c_status_t mcp_init_status = MCP23017_Initialize(&i2c_host);
    ...
    uint8_t mcp_port_a_value = 0xffU;
    i2c_status_t status;
    i2c_buffer[0] = BANKED_GPIOB;
    i2c_buffer[1] = mcp_port_a_value;
    status = i2c_writeClient(&i2c_host, MCP23017_ADDR, i2c_buffer, 2);
    if (status == I2C_SUCCESS)
    {
        status = i2c_waitComplete(&i2c_host);
    }
    if (status == I2C_SUCCESS)
    {
        uint8_t reg = BANKED_GPIOA;
        status = i2c_writeReadClient(&i2c_host, MCP23017_ADDR, reg, i2c_buffer, 1);
    }
    if (status == I2C_SUCCESS)
    {
        status = i2c_waitComplete(&i2c_host);
    ...
    if (status == I2C_SUCCESS)
    {
        ...
        if (status == I2C_SUCCESS)
        {
            (void)i2c_waitComplete(&i2c_host);
        }
    }
...
void APP_Service(void)
{
    ...
    if (update_now)
    {
        ...
        status = i2c_writeClient(&i2c_host, MCP23017_ADDR, i2c_buffer, 1);
        if (status == I2C_SUCCESS)
        {
            status = i2c_waitComplete(&i2c_host);
        }
        if (status == I2C_SUCCESS)
        {
            status = i2c_writeReadClient(&i2c_host, MCP23017_ADDR, i2c_buffer[0], i2c_buffer, 1);
        }
        if (status == I2C_SUCCESS)
        {
            status = i2c_waitComplete(&i2c_host);
