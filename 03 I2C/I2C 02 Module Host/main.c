...
volatile bool s_porta_value_changed =
    false; // Flag indicating if the last known value of Port A has changed
...
void main(void)
{
    ...
    while (1)
    {
        if (s_porta_value_changed)
        {
            ...
            status = I2C_ReadRegister(&i2c_host, MCP23017_ADDR, BANKED_GPIOA,
    ...
    /// @param  None
    /// @return None
    /// @note This simply sets a flag to indicate that the port A value has changed. The main loop
    /// will handle the I2C read and write operations.
    void __interrupt(irq(IRQ_INT1), low_priority) Extern_ISR(void)
    {
        s_porta_value_changed = true; // Flag to indicate that the port A value has changed
        PIR6bits.INT1IF = 0;
    }
