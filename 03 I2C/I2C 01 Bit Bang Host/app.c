/* *****************************************************************************************
 *   File Name: app.c
 *   Description: Bit bang I2C host implementation for PIC18F__Q43.
 ...
 *
 *   This is the demonstration application implementation for the I2C 01 Bit Bang Host
 *   example. It initializes the system, sets up UART1 for debug output, and configures
 *   the I2C bit bang host interface.
 *
...
void __interrupt(irq(IRQ_INT1), low_priority) Extern_ISR(void)
{
    ...
    i2c_status_t status = I2C_SendBytes(&i2c_host, MCP23017_ADDR, &reg_addr, 1);
    if (status == I2C_SUCCESS)
    {
        status = I2C_ReceiveByte(&i2c_host, MCP23017_ADDR, &mcp_port_a_value, false);
    }
    ...
    if (status == I2C_SUCCESS)
    {
        ...
        status = I2C_SendBytes(&i2c_host, MCP23017_ADDR, write_buf, 2);
    }
...
///       bit bang host interface. It also prints status messages to the console.
void APP_Initialize(void)
{
    printf("I2C 01 Bit Bang Host\r\n");
    ...
    // Initialize I2C bit bang host for 100kHz bus speed
    if (I2C_Initialize(&i2c_host, 100) != I2C_SUCCESS)
    {
    ...
    if (MCP23017_Initialize(&i2c_host) != I2C_SUCCESS)
    {
    ...
    uint8_t reg_addr = BANKED_GPIOA;
    i2c_status_t status = I2C_SendBytes(&i2c_host, MCP23017_ADDR, &reg_addr, 1U);
    if (status == I2C_SUCCESS)
    {
        status = I2C_ReceiveByte(&i2c_host, MCP23017_ADDR, &mcp_port_a_value, false);
    }
    if (status == I2C_SUCCESS)
    {
        ...
        (void)I2C_SendBytes(&i2c_host, MCP23017_ADDR, write_buf, 2U);
    }
