...
i2c_status_t i2c_writeReadSlave(i2c_handle_t *handle, uint16_t address, const uint8_t *write_data,
...
void __attribute__((weak)) __interrupt(irq(IRQ_I2C1), high_priority) i2c1_generalISR(void)
{
    ...
    // Check the specific flags in I2C1PIR to determine the cause of the interrupt and take appropriate action
    if (PIR7bits.I2C1IF) // Check for general I2C event interrupt
    {
        PIR7bits.I2C1IF = 0; // Clear the general I2C event interrupt flag
        // Handle the specific I2C event based on the current operation and flags
        // (e.g., check for byte count complete, start/stop condition detected, etc.)
        // and take appropriate action (e.g., update status, signal waiting tasks, etc.)
    }
}
