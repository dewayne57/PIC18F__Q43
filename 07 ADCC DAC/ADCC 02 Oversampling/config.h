...
#define _XTAL_FREQ 64000000 // Define the system clock frequency for delay functions
...
#define ADCC_VREF_POSITIVE_MV 5000
...
#define ADCC_MV_TO_COUNTS(mv) ((((unsigned long)(mv) * ADCC_MAX_COUNTS) + (ADCC_VREF_POSITIVE_MV / 2)) / ADCC_VREF_POSITIVE_MV)
#define ADCC_COUNTS_TO_MV(counts) ((((unsigned long)(counts) * ADCC_VREF_POSITIVE_MV) + (ADCC_MAX_COUNTS / 2)) / ADCC_MAX_COUNTS)

// Number of samples the ADCC hardware accumulates and averages per result.
// Must be a power of 2 between 2 and 256.
#define ADCC_OVERSAMPLE_COUNT 32
